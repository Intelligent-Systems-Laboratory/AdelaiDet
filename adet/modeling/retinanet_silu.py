...
def activation(activation_fun):
    """Helper for building an activation layer."""
    activation_fun = activation_fun.lower()
    if activation_fun == "relu":
        return nn.ReLU(inplace=cfg.MODEL.RETINANET.ACTIVATION_INPLACE)
...
class RetinaNet_SiLu(nn.Module):
    """
...
class RetinaNetHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for in_channels, out_channels in zip([input_shape[0].channels] + conv_dims, conv_dims):
            ...
            cls_subnet.append(activation(activation_fun))
            bbox_subnet.append(
                nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
            )
            if norm:
                bbox_subnet.append(get_norm(norm, out_channels))
            bbox_subnet.append(activation(activation_fun))
    ...
    def from_config(cls, cfg, input_shape: List[ShapeSpec]):
        ...
        return {
            ...
            "activation_fun": cfg.MODEL.RETINANET.ACTIVATION_FUN,
        }
